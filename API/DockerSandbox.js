 ...
 * @param {String} folder: The name of the folder that would be mounted/shared with Docker container,
 *        this will be concatenated with path
 ...
 * @param {String} output_command: Used in case of compilers only, to execute the object code,
 *             send " " in case of interpretors
*/
var DockerSandbox = function(timeout_value,path,folder,vm_name,compiler_name,file_name,code,output_command,languageName,e_arguments,stdin_data) {
    ...
    console.log('DockerSandbox: Create');
...
{
    console.log('DockerSandbox: Prepare');
    var exec = require('child_process').exec;
    var fs = require('fs');
    var sandbox = this;
    var folderPath = this.path + this.folder;
    var payloadPath = this.path + '/Payload';

    var command1 = 'mkdir ' + folderPath;
    var command2 = 'cp ' + payloadPath + '/* ' + folderPath;
    var command3 = 'chmod 777 ' + folderPath;

    exec(command1 + ' && ' + command2 + ' && ' + command3 ,function(st) {
...
DockerSandbox.prototype.execute = function(success) {
    console.log('DockerSandbox: Execute');
    var exec = require('child_process').exec;
    var fs = require('fs');
    var myC = 0; //variable to enforce the timeout_value
    var sandbox = this;

    var shellScriptPath = this.path+'DockerTimeout.sh';
    var tTag = this.timeout_value + 's';
    var eTag = '-e \'NODE_PATH=/usr/local/lib/node_modules\'';
    var uTag = '-u mysql';
    var vTag = '-v  "' + this.path + this.folder + '":/usercode ' + this.vm_name;
        vTag += ' /usercode/script.sh ' + this.compiler_name + ' ' + this.file_name;
        vTag += ' ' + this.output_command+ ' ' + this.extra_arguments;

    //this statement is what is executed
    var st = 'source ' + shellScriptPath + ' ' + tTag +' ' + uTag + ' ' + eTag + ' -i -t ' + vTag;
    ...
    var intid = setInterval(function() {
            ...
            fs.readFile(sandbox.path + sandbox.folder + '/completed', 'utf8', function(err, data) {
                if (err) console.log('Error Reading Completed File', err);
                ...
                //if time is up. Save an error message to the data variable
                    ...
                    fs.readFile(sandbox.path + sandbox.folder + '/logfile.txt', 'utf8', function(err, data){
                        if (err) console.log('Error Reading Log', err);
                        if (!data) data = '';
                        data += '\nExecution Timed Out';
                        console.log('Timed Out: ' + sandbox.folder + ' '+ sandbox.langName);
                        fs.readFile(sandbox.path + sandbox.folder + '/errors', 'utf8', function(err2, data2) {
                            if (err2) console.log('Error Reading File', err2);
                            if(!data2) data2 = '';
